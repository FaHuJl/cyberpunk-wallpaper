...
// Yandex Music widget state
var ymTitle = "Трек не выбран";
var ymArtist = "Исполнитель";
var ymProgress = 0; // 0-100 процент прогресса
var ymCover = "assets/img/ym-cover-placeholder.jpg";
...
function livelyPropertyListener(name, val) {
    ...
    if (name === "ymProgress") {
        ymProgress = Math.max(0, Math.min(100, val));
    }
    if (name === "ymCover") {
        ymCover = val || ymCover;
    }
...
function main() {
    ...
    // Yandex Music widget render
    $('#ym-title').html(ymTitle);
    $('#ym-artist').html(ymArtist);
    $('#ym-cover-img').attr('src', ymCover);
    $('#ym-progress-bar').css('width', ymProgress + "%");
}
